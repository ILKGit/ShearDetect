...
def train_one_epoch(model, optimizer,lr_scheduler, data_loader, val_loader, device, epoch, print_freq, scaler=None):
    ## Training step
    model.train()
    metric_logger_train = utils.MetricLogger(delimiter="  ")
    metric_logger_train.add_meter("lr", utils.SmoothedValue(window_size=1, fmt="{value:.6f}"))
    header = f"Epoch: [{epoch}]"

    for images, targets in metric_logger_train.log_every(data_loader, print_freq, header):
    ...
    with torch.no_grad():
        ...
        for images, targets in metric_logger_val.log_every(val_loader, print_freq, header):
            images = list(image.to(device) for image in images)
            targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
            with torch.cuda.amp.autocast(enabled=scaler is not None):
            ...
            metric_logger_val.update(loss=losses_reduced_val, **loss_dict_reduced)

    
    return metric_logger_train, metric_logger_val
